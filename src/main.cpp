...
ReactESP app([] () {
  ...
  auto* pINA219busVoltage = new INA219value(pINA219, bus_voltage, read_delay, "/battery/busVoltage");
      
      pINA219busVoltage->connectTo(new SKOutputNumber("electrical.battery.busVoltage"));

  // Do the same for the shunt voltage.
  auto* pINA219shuntVoltage = new INA219value(pINA219, shunt_voltage, read_delay, "/battery/shuntVoltage");
      
      pINA219shuntVoltage->connectTo(new SKOutputNumber("electrical.battery.shuntVoltage"));

  // Do the same for the current (amperage).
  auto* pINA219current = new INA219value(pINA219, current, read_delay, "/battery/current");
      
      pINA219current->connectTo(new SKOutputNumber("electrical.battery.current"));   

  // Do the same for the power (watts).
  auto* pINA219power = new INA219value(pINA219, power, read_delay, "/battery/power");
      
      pINA219power->connectTo(new SKOutputNumber("electrical.battery.power"));  

  // Do the same for the load voltage.
  auto* pINA219loadVoltage = new INA219value(pINA219, load_voltage, read_delay, "/battery/loadVoltage");
      
      pINA219loadVoltage->connectTo(new SKOutputNumber("electrical.battery.loadVoltage"));         
